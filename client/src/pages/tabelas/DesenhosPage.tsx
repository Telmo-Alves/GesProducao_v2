  ...
  return (
    <div className="p-6">
      ...
      <form onSubmit={submit} className="bg-white p-4 rounded shadow mb-4 grid grid-cols-1 md:grid-cols-4 gap-3 items-end">
        ...
        <div><label className="block text-sm text-gray-600">Cliente</label><input type="number" className="border rounded px-3 py-2" value={form.cliente} onChange={(e) => setForm(f => ({ ...f, cliente: e.target.value === '' ? '' : Number(e.target.value) }))} /></div>
