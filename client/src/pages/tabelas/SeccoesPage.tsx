...
const SeccoesPage: React.FC = () => {
  const [items, setItems] = useState<SeccaoOption[]>([]);
  const [search, setSearch] = useState('');
  const [page, setPage] = useState(1);
  const [totalPages, setTotalPages] = useState(1);
  const [loading, setLoading] = useState(false);
  const [form, setForm] = useState<{ seccao: number | ''; descricao: string; ordem: number | ''; situacao: string }>({ seccao: '', descricao: '', ordem: 1, situacao: 'ACT' });
  const [editing, setEditing] = useState<number | null>(null);

  const load = async () => {
    setLoading(true);
    try {
      const { data } = await tabelasApi.listSeccoes({ page, limit: 10, search });
      const payload = data.data as PagedResult<SeccaoOption>;
  ...
  const submit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (form.seccao === '' || !form.descricao) return;
    if (editing === null) {
      await tabelasApi.createSeccao({ seccao: Number(form.seccao), descricao: form.descricao, ordem: form.ordem === '' ? undefined : Number(form.ordem), situacao: form.situacao });
    } else {
      await tabelasApi.updateSeccao(editing, { descricao: form.descricao, ordem: form.ordem === '' ? undefined : Number(form.ordem), situacao: form.situacao });
    }
    setForm({ seccao: '', descricao: '', ordem: 1, situacao: 'ACT' });
    setEditing(null);
    await load();
  };

  const startEdit = (item: SeccaoOption) => {
    setEditing(item.seccao);
    setForm({ seccao: item.seccao, descricao: item.descricao, ordem: item.ordem ?? 1, situacao: item.situacao || 'ACT' });
  };

  const remove = async (seccao: number) => {
    if (!confirm('Remover secção?')) return;
    await tabelasApi.deleteSeccao(seccao);
    await load();
  };
  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-4">
        <h1 className="text-2xl font-semibold">Secções</h1>
      </div>
      ...
      <form onSubmit={submit} className="bg-white p-4 rounded shadow mb-4 grid grid-cols-1 md:grid-cols-5 gap-3 items-end">
        ...
        <div>
          <label className="block text-sm text-gray-600">Ordem</label>
          <input type="number" className="border rounded px-3 py-2" value={form.ordem}
            onChange={(e) => setForm(f => ({ ...f, ordem: e.target.value === '' ? '' : Number(e.target.value) }))} />
        </div>
        <div>
        ...
        <div className="md:col-span-5 flex gap-3 items-end">
          ...
          {editing !== null && (
            <button type="button" onClick={() => { setEditing(null); setForm({ seccao: '', descricao: '', ordem: 1, situacao: 'ACT' }); }} className="px-4 py-2 rounded border">
              Cancelar
            </button>
          )}
        </div>
      ...
      <div className="bg-white rounded shadow overflow-x-auto">
        <table className="w-full">
          ...
          <tbody>
            {loading ? (
              <tr><td className="px-4 py-3" colSpan={5}>A carregar...</td></tr>
            ) : items.map((it) => (
              <tr key={it.seccao} className="border-t">
                <td className="px-4 py-2">{it.seccao}</td>
                <td className="px-4 py-2">{it.descricao}</td>
                <td className="px-4 py-2">{it.ordem ?? '-'}</td>
                ...
                <td className="px-4 py-2">
                  ...
                  <button className="text-red-600 hover:text-red-800" onClick={() => remove(it.seccao)}><Trash2 size={16} /></button>
                </td>
              </tr>
            ))}
          </tbody>
