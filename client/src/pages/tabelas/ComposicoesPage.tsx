...
export const ComposicoesPage: React.FC = () => {
  const [items, setItems] = useState<ComposicaoOption[]>([]);
  ...
  const load = async () => {
    setLoading(true);
    try {
      const { data } = await tabelasApi.listComposicoes({ page, limit: 10, search });
      const payload = data.data as PagedResult<ComposicaoOption>;
