...
const EstadosPage: React.FC = () => {
  ...
  const [form, setForm] = useState<{ id: number | ''; descricao: string; movimenta: string; situacao: string }>({ id: '', descricao: '', movimenta: 'N', situacao: 'ACT' });
  const [editing, setEditing] = useState<number | null>(null);

  const load = async () => { setLoading(true); try { const { data } = await tabelasApi.listEstados({ page, limit: 10, search }); const payload = data.data; setItems(payload.data); setTotalPages(payload.totalPages); } finally { setLoading(false); } };
